...
| Tagline | **[x] Confirmado (2026-07-06, revisado)** — *"Listens to your body, engineers your training."* En inglés, deliberado (misma lógica de dos capas de idioma que el nombre, ver nota abajo). Reemplaza la versión anterior *"Your body leads. The program follows."* (2026-07-04): esa primera versión sonaba a que el programa se limita/subordina al cuerpo; la nueva mantiene la escucha del cuerpo como input pero dos verbos activos ("listens"/"engineers") dejan explícito que el sistema construye y empuja el entrenamiento, no solo lo restringe — coherente con el manifesto (empujar límites de forma segura, no evitarlos). Corrección de drift: la línea anterior de esta tabla decía "aplicado en `layout.tsx` y `AppNav.tsx`", pero eso nunca fue cierto — el tagline solo ha existido en este doc y en `PENDIENTES.md`; `layout.tsx` usa su propia descripción en español ("Tu sistema de entrenamiento personal") y `AppNav.tsx` solo renderiza "AST", sin la frase en inglés. Sigue sin estar en UI — pendiente de aplicar cuando el logo (abajo) se integre |
| Logo / isotipo | **[DRAFT] Explorado en ChatGPT (2026-07-06)** — monograma A-S-T angular, ámbar sobre negro, resuelve el problema de legibilidad de la S detectado en el borrador v1 (`ast-monogram-v1-DRAFT.svg`, ya no se lee como Z). Pendiente traerlo a código como asset final (SVG/PNG) y decidir dónde vive en la UI (hoy la "marca" en producto sigue siendo solo el wordmark de texto en `.sidebar-brand`) |
